...
> Worked examples that show each field branch in action — how `--branch <id>`
> tunes the same TRIZ core to an ultra-specific task in that field. Every example
> runs against the live tools, so the router output matches what you would see.
...
**Branch detection:** The router matched `torque`, `gearbox`, `housing` and the
`--branch mechanical` filter isolates the `Mechanical TRIZ` domain rule — the
business/software/rehab rules are skipped.
...
Top methods: Engineering Contradiction + 40 Inventive Principles (7) — the
"must ... but" trade-off is detected as a contradiction — then Mechanical TRIZ
(2).

**Parameter translation:** `Force` → *torque / load / clamping force*; `Weight
of moving object` → *mass / moment of inertia of the moving part*; `Stress or
pressure` → *contact stress / preload*. Improving = 10 Force, worsening = 1
Weight of moving object.

**Solution via soft principles:**
- `IP-35` (change material state) — case-harden the gear teeth so each tooth
...
**Result:** case-harden the teeth `[IP-35 Parameter change]` and split the
...
**Branch detection:** The router matched `model`, `features`, `training`; the
`--branch datascience` filter isolates the `Data Science TRIZ` rule.
...
Top methods: Engineering Contradiction + 40 Inventive Principles (7) — the
"must ... but ... slow" trade-off reads as a contradiction (a physical
contradiction is also suspected near "must be") — then Data Science TRIZ (2).

**Parameter translation:** `Accuracy of measurement` → *model accuracy / F1 /
precision-recall*; `Speed` → *training time / inference latency / throughput*;
`Amount of substance` → *dataset size / feature count*. Improving = 28
...
**Branch detection:** The router matched `landing page`, `convert`, `sign-ups`;
the `--branch marketing` filter isolates the `Marketing TRIZ` rule.
...
Top methods: Engineering Contradiction + 40 Inventive Principles (7) — the
conversion vs. friction trade-off reads as a contradiction — then Marketing
TRIZ (2).
...
**Branch detection:** The router matched `lead time`, `safety stock`,
`inventory`; the `--branch supplychain` filter isolates the `Supply Chain TRIZ`
rule.
...
Top methods: Engineering Contradiction + 40 Inventive Principles (7) — lead
time vs. inventory is a classic contradiction — then Supply Chain TRIZ (2).

**Parameter translation:** `Speed` → *lead time / delivery speed / throughput*;
`Amount of substance` → *inventory volume / order quantity*; `Loss of time` →
*idle time / waiting at the dock*. Improving = 9 Speed, worsening = 26 Amount of
substance.
