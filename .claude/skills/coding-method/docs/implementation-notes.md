...
- One coding problem-solving method from three sources: **ponytail** (the lazy
  default), **TRIZ** (the stuck-state engine), and the **distilled method
  catalog** (the standard-moves catalog).
...
2. **Lazy Ideality is one method, not a mashup.** Ponytail decides *what to
   build* (skip everything unneeded); the method catalog decides *how the thing
   hides its complexity* (deep modules, seams, strategy catalogs); TRIZ
   dissolves the *contradiction* when the catalog can't reach the one-liner. IFR
   is the generation north-star; trade-off analysis is the evaluation
   instrument.
...
4. **Cards → references.** The 16 reference files were rendered from 14
   distilled source-material cards (produced by a design workflow over the
   converted source material in `CodingBooks-md`; the data-systems card was
   added 2026-08-02 via `how-to-add-book.md`). Merges:
   `algorithm-strategies.md` (strategy + computation-model cards), `red-flags.md`
   (three review catalogues), `construction-checklist.md` (two construction
   catalogues). The two cross-cutting files (`method-map.md`, `triz-for-code.md`)
...
## Extension points
- New source material → `how-to-add-book.md`: convert → distill → render →
  update index + curriculum. Done: data-systems → `data-systems.md`
  (2026-08-02). Planned: the algorithm-strategies and refactoring cards
  (merge targets `algorithm-strategies.md` and
...
- The missing curriculum steps are marked with interim substitutes; none of
  them block the data-heavy fork (the data-systems card is already in).
- Source material in `CodingBooks-md` is copyrighted — never commit it.
