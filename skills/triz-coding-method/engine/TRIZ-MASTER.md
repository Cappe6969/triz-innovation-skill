...
Persist the analysis as a reusable case file only in a directory you select: `python skills/triz-coding-method/engine/scripts/triz_case_template.py "Short problem title" --output-dir ./my-cases`. Fill it as you run the pipeline.
...
3. **Look up the cell.** Run `python skills/triz-coding-method/engine/scripts/triz_matrix.py <improving_id> <worsening_id>`. The matrix returns a shortlist of inventive principle numbers.
...
**Procedure:** Generate the worksheet with `python skills/triz-coding-method/engine/scripts/triz_ariz.py "problem title"` and fill every part in writing. ARIZ is a worksheet, not a chat — the discipline of writing each step is what produces the insight.
...
The `triz-innovation` skill includes helper scripts that automate lookups, generate worksheets, and evaluate solutions. All live under `skills/triz-coding-method/engine/scripts/`.
...
| `triz_case_template.py` | Creates a pre-filled markdown case in an explicit output directory: `python triz_case_template.py "Short problem title" --output-dir ./my-cases` |
...
This table maps every method section in the master document to the source file(s) it consolidates. Every referenced path exists on disk under `skills/triz-coding-method/engine/`.
...
The 18 reference files in `skills/triz-coding-method/engine/references/` and the pipeline in `skills/triz-coding-method/engine/SKILL.md`. Each is an original operational rewrite — condensed, procedural, and designed for use during a TRIZ session. This master document consolidates and enriches them without altering their core concepts.

### Source material (conceptual reference only)

The published sources listed in `docs/bibliography.md` informed the synthesis as
...
**TRIZ Prompt Engineering upstream repository** — A structured collection of TRIZ prompts in XML format, developed by the ccTOPP / TRIZ-prompt-engineering project and released under the **MIT License**. The following files were consulted for additional structure, checklists, and examples:
