...
**Almost-IFR:** divide-and-conquer O(n log n) con strip limitata da `delta`.
Un semplice sort seguito dal confronto dei soli punti adiacenti non è corretto
nel caso 2D generale.
...
- **Test/esperimento:** property test vs brute force su fixture casuali e casi
  degeneri; benchmark separato su N=10⁵.
- **Criterio di successo/fallimento:** stessa distanza e una coppia valida
  rispetto all'oracolo su ogni fixture; crescita compatibile con O(n log n).
...
Caso metodologico, non implementazione eseguita: nessun tempo o conteggio di
righe è stato misurato. La scelta verificabile è divide-and-conquer O(n log n);
`sort+scan` sui soli adiacenti è stato escluso perché non corretto in 2D.

## Follow-up

Implementare prima l'oracolo O(n²), poi confrontarlo con la versione
divide-and-conquer su input generati e pubblicare soltanto misure riproducibili.
